...
<body>
	...
	<script type="text/javascript">
		...
		function fromLineStrToLineArrGoBack(lineStr)
		{
			var lineArr_temp = lineStr.split(",");
			var lineArr = new Array();
			var temp_length = lineArr_temp.length;
			for (var i = 0; i < lineArr_temp.length; i += 2)
			{
				lineArr[i / 2] = new Array();
				lineArr[i / 2][0] = lineArr_temp[i];
				lineArr[i / 2][1] = lineArr_temp[i + 1];
				lineArr[temp_length - 1 - i / 2] = new Array();
				lineArr[temp_length - 1 - i / 2][0] = lineArr_temp[i];
				lineArr[temp_length - 1 - i / 2][1] = lineArr_temp[i + 1];
			}
			return lineArr;
		}
		...
		$(function()
		{
			...
			lineArr = fromLineStrToLineArrGoBack(lineArr_Str_2);
			var circle_center = [ 118.00952, 36.857722 ];
			var monitorObject = new MonitorObject(car, lineArr, circle_center, 2);
			...
			$('#divId').timer(
			{
				...
				callback : function()
				{
					$("#count_speed_flag").text(parseInt($("#count_speed_flag").text()) + 10);
					//创建数组中的车辆运动点
